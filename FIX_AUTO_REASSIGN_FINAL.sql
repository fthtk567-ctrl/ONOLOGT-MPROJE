-- YENIDEN ATAMA TRİGGER'INI İYİLEŞTİR (notifications type constraint fix)
...
BEGIN
  ...
  IF NEW.status = 'pending' AND NEW.courier_id IS NULL AND NEW.rejected_by IS NOT NULL THEN
    ...
    -- Müsait kuryeyi bul (red eden hariç) - KONUM KONTROLÜ YOK
    ...
    WHERE 
      ...
      AND status = 'approved'
    ORDER BY RANDOM() -- Rastgele sırala
    LIMIT 1;
    ...
    ELSE
      ...
      -- Merchant'a bildirim gönder (type='delivery' kullan, constraint hatası yok)
      INSERT INTO notifications (
        user_id,
        title,
        message,
...
-- Başarılı mesajı
SELECT 'Trigger başarıyla güncellendi - type constraint fix edildi!' as status;
